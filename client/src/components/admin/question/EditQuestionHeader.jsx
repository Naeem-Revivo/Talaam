...
const EditQuestionHeader = ({ 
  ...
  const defaultTitleKey = titleKey || "admin.addNewQuestion.hero.title";
  const defaultCancelKey = cancelKey || "admin.addNewQuestion.buttons.cancel";
  const defaultSubmitKey = submitKey || updateKey || "admin.addNewQuestion.buttons.submit";
  const defaultRejectFlagKey = rejectFlagKey || "gatherer.addNewQuestion.rejectFlag.button";
  ...
  return (
    <header className="flex flex-col md:flex-row justify-between gap-4 items-start md:items-center mb-10">
      <h1 className="font-archivo text-[24px] md:text-[36px] font-bold leading-[28px] md:leading-[40px] text-oxford-blue">
        {t(defaultTitleKey)} - {t(updateKey || "gatherer.addNewQuestion.update") || 'Edit'}
      </h1>
      <div className="flex flex-wrap gap-2 md:gap-4 w-full md:w-auto">
        {isFlagged && onRejectFlag && (
          <button
            type="button"
        ...
        >
          {submitting ? (t("gatherer.addNewQuestion.updating") || "Updating...") : t(defaultSubmitKey)}
        </button>
      </div>
    </header>
  );
