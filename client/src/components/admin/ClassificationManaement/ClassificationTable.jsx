...
const MobileCard = ({ item, columns, onView, onEdit }) => {
  const displayColumns = columns.slice(0, -1); // Exclude Actions column
  
  // Get the field key from column name
  const getFieldKey = (columnName) => {
    return columnName.toLowerCase().replace(/ /g, "");
  };

  // Get first two fields for header (usually name and description)
  const headerFields = displayColumns.slice(0, 2);
  const detailFields = displayColumns.slice(2);
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            {headerFields.map((column, idx) => {
              ...
              if (idx === 0) {
                return (
                  <p key={column} className="text-[16px] font-archivo font-semibold leading-[20px]">
                    {value}
                  </p>
                );
              } else if (idx === 1) {
                return (
                  <p key={column} className="mt-1 text-[14px] font-roboto text-[#6B7280]">
                    {value}
                  </p>
                );
              }
        ...
        {detailFields.length > 0 && (
          <div className="flex flex-col gap-2 text-[14px] font-roboto text-[#1F2937]">
            {detailFields.map((column) => {
              ...
              return (
                <p key={column}>
                  <span className="font-semibold">{column}:</span> {value}
                </p>
              );
            })}
          </div>
