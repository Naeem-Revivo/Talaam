    ...
    <section className="flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-center lg:justify-between lg:gap-4 xl:gap-6 2xl:gap-8">
      <label className="relative flex items-center w-full lg:max-w-[400px] xl:max-w-[500px] 2xl:max-w-[600px]">
        ...
        <input
          ...
          className="h-[46px] sm:h-[50px] w-full rounded-[12px] sm:rounded-[14px] border border-transparent placeholder:text-dark-gray placeholder:text-xs sm:placeholder:text-sm bg-white pl-10 sm:pl-12 pr-3 sm:pr-4 text-sm sm:text-base font-roboto text-oxford-blue shadow-filter focus:border-[#032746] focus:outline-none focus:ring-2 focus:ring-[#D6E3F0]"
        />
        <svg
          width="18"
          height="18"
          ...
          className="pointer-events-none absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-[18px] h-[18px] sm:w-[21px] sm:h-[21px]"
        >
          <path
      ...
      <div className="relative flex justify-start lg:hidden" ref={mobileFilterRef}>
        <button
          ...
          className="flex h-[46px] sm:h-[48px] min-w-[140px] sm:min-w-[150px] items-center justify-between rounded-[12px] sm:rounded-[14px] border border-transparent bg-white px-3 sm:px-4 text-[14px] sm:text-[16px] font-archivo font-semibold text-oxford-blue shadow-filter-hover focus:border-[#032746] focus:outline-none focus:ring-2 focus:ring-[#D6E3F0]"
          aria-expanded={isMobileFiltersOpen}
        >
          <span className="flex items-center gap-1.5 sm:gap-2">
            <svg width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-[18px] h-[18px] sm:w-[20px] sm:h-[20px]">
              <path
          ...
          <img src={dropdownArrow} alt="" className="w-3 h-3 sm:w-auto sm:h-auto" />
        </button>

        {isMobileFiltersOpen ? (
          <div className="absolute right-0 top-full z-20 mt-2 sm:mt-3 w-[calc(100vw-2rem)] max-w-[280px] sm:w-[250px] rounded-[14px] sm:rounded-[18px] border border-[#E5E7EB] bg-white p-3 sm:p-4 shadow-dropdown">
            <div className="flex flex-col gap-3 sm:gap-4 text-oxford-blue">
              <section className="flex flex-col gap-2">
                <p className="text-[11px] sm:text-[13px] font-archivo font-semibold uppercase tracking-[0.08em] text-dark-gray">
                ...
                <div className="flex flex-col gap-1.5 sm:gap-2">
                  {planOptions
                    ...
                    .map((option) => {
                      ...
                      return (
                        <label key={option.value} className="flex items-center gap-2 text-[13px] sm:text-[14px] font-roboto">
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() => onPlanChange?.(isChecked ? "" : option.value)}
                            className="h-3.5 w-3.5 sm:h-4 sm:w-4 rounded border border-[#9CA3AF] text-[#ED4122] focus:ring-[#ED4122]"
                          />
                          {option.label}
                        </label>
                      );
            ...
            <div className="mt-4 sm:mt-6 flex flex-col gap-2 sm:gap-3">
              <button
                ...
                className="h-[40px] sm:h-[44px] rounded-[10px] sm:rounded-[12px] border border-[#032746] bg-white text-[14px] sm:text-[15px] font-archivo font-semibold text-oxford-blue transition hover:bg-[#F2F5FA]"
              >
              ...
              <button
                ...
                className="h-[40px] sm:h-[44px] rounded-[10px] sm:rounded-[12px] bg-[#ED4122] text-[14px] sm:text-[15px] font-archivo font-semibold text-white transition hover:bg-[#d43a1f]"
              >
      ...
      {/* Desktop filters */}
      <div className="hidden lg:flex lg:flex-row lg:items-center lg:gap-2 xl:gap-3 2xl:gap-4 lg:flex-shrink-0">
        <div className="flex-shrink-0 w-[150px] xl:w-[170px] 2xl:w-[180px]">
          <Dropdown
