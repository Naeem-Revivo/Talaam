...
const TableRow = ({ item, columns, onEdit, onDelete, onTogglePublish, onView }) => {
  return (
    <tr className="hidden border-b border-[#E5E7EB] bg-white text-oxford-blue last:border-none md:table-row">
      {columns.slice(0, -1).map((column) => {
        ...
        if (column.key === 'status') {
          ...
          const isExpired = value.toLowerCase() === 'expired';
          return (
            <td key={column.key} className="px-6 py-4 text-center">
              <span className={`inline-block px-[12px] py-[5px] rounded-md text-[14px] leading-[100%] font-normal font-roboto ${isActive ? 'bg-[#FDF0D5] text-[#ED4122]' :
                ...
                isExpired ? 'bg-[#C6D8D3] text-oxford-blue' :
        ...
        // Special rendering for publish column
            ...
            <td key={column.key} className="px-6 py-4 text-center">
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  item.isPublished ? 'bg-[#EF4444]' : 'bg-[#E5E7EB]'
                }`}
...
const MobileCard = ({ item, columns, onEdit, onDelete, onTogglePublish, onView }) => {
    ...
    <article className="flex flex-col rounded-[8px] border border-[#E5E7EB] bg-white shadow-sm md:hidden overflow-hidden">
      <div className="flex flex-col gap-2 px-4 py-3 text-oxford-blue">
        {displayColumns.map((column) => {
          ...
          // Skip status field in top section (it's shown in bottom section)
          if (column.key === 'status') {
            return null;
          }

          // Skip publish field in top section (it's shown in bottom section)
          if (column.key === 'publish') {
            return null;
          }
      ...
      {/* Bottom section with status, publish, and action buttons */}
      <div className="flex items-center justify-between border-t border-[#E5E7EB] px-4 py-2.5">
        <div className="flex items-center gap-3">
          ...
          {/* Publish toggle */}
          {(() => {
            const publishColumn = displayColumns.find(col => col.key === 'publish');
            if (publishColumn && onTogglePublish) {
              return (
                <div className="flex items-center gap-2">
                  <span className="text-[12px] font-normal text-[#6B7280]">{publishColumn.label}:</span>
                  <button
                    type="button"
                    onClick={() => onTogglePublish?.(item)}
                  ...
                  >
                    <span
                      className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
                        item.isPublished ? 'translate-x-5' : 'translate-x-0.5'
                      }`}
                    />
                  </button>
                </div>
...
const Loader = () => (
  <div className="flex items-center justify-center py-20">
    <div className="relative">
      <div className="w-12 h-12 border-4 border-[#E5E7EB] border-t-[#ED4122] rounded-full animate-spin"></div>
    </div>
  </div>
);
