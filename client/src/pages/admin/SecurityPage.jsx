...
export default function SecuritySettingsPage() {
  ...
  const loginColumns = [
  { label: t("admin.rolesPermissions.loginHistory.table.columns.user"), key: "user" },
  { label: t("admin.rolesPermissions.loginHistory.table.columns.ipAddress"), key: "ipaddress" },
  { label: t("admin.rolesPermissions.loginHistory.table.columns.date"), key: "date" },
  { label: t("admin.rolesPermissions.loginHistory.table.columns.status"), key: "status" }
];
    ...
    <div className="max-w-[1200px] mx-auto py-10 px-6">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center mb-[30px]">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {t("admin.security.buttons.rolesAndPermissions")}
          </button>
          <button
            type="button"
          ...
          >
            {t("admin.security.buttons.auditLogs")}
          </button>
        </div>
      </header>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl pt-8 pb-10 space-y-10">
        ...
        <section className="px-8 ">
          ...
          <p className="text-[16px] leading-[100%] font-normal text-oxford-blue font-roboto">
            {t("admin.security.sections.securityAndAuthentication.description")}
          </p>
        </section>

        {/* Password Policy */}
        <section className="border-t pt-6 px-8">
          <h3 className="text-[20px] leading-[100%] font-bold text-oxford-blue mb-3">
            {t("admin.security.sections.passwordPolicy.title")}
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-[10px] text-[16px] font-normal leading-[100%] text-blue-dark">
            <label className="flex items-center gap-2">
              <input type="checkbox" />{" "}
              {t("admin.security.sections.passwordPolicy.minimum8Chars")}
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" />{" "}
              {t("admin.security.sections.passwordPolicy.atLeast1Uppercase")}
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" />{" "}
              {t("admin.security.sections.passwordPolicy.atLeast1Lowercase")}
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" />{" "}
              {t("admin.security.sections.passwordPolicy.atLeast1Number")}
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" />{" "}
              {t("admin.security.sections.passwordPolicy.atLeast1SpecialChar")}
            </label>
          </div>

          <div className="flex items-center justify-between mt-[65px] text-[14px] leading-[100%] text-blue-dark">
            <span>
              {t("admin.security.sections.passwordPolicy.forcePasswordChange")}
            </span>
            <button
              onClick={() => setPasswordPolicy(!passwordPolicy)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                passwordPolicy ? "bg-[#EF4444]" : "bg-[#E5E7EB]"
              }`}
            >
              <span
        ...
        {/* 2FA */}
        <section className="border-t pt-6 px-8">
          <h3 className="text-[20px] leading-[100%] font-bold text-oxford-blue mb-9">
            {t("admin.security.sections.twoFactorAuth.title")}
          </h3>

          <div className="flex items-center justify-between mb-10">
            <p className="text-[16px] leading-[100%] font-normal text-oxford-blue">
              {t("admin.security.sections.twoFactorAuth.enable2FA")}
            </p>
            <button
              onClick={() => setTwoFactor(!twoFactor)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                twoFactor ? "bg-[#EF4444]" : "bg-[#E5E7EB]"
              }`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
          ...
          <div className="flex items-center justify-between">
            <p className="text-[16px] leading-[100%] font-normal text-oxford-blue">
              {t("admin.security.sections.twoFactorAuth.viewOrResetDevices")}
            </p>
            <button className="text-[14px] leading-[100%] font-normal text-[#ED4122] bg-[#FDF0D5] px-[10px] py-[5px] rounded-md">
              {t("admin.security.sections.twoFactorAuth.manage")}
            </button>
          </div>
        </section>
        ...
        <section className="border-t pt-6 px-8">
          <h3 className="text-[20px] leading-[100%] font-bold text-oxford-blue mb-[30px]">
            {t("admin.security.sections.sensitiveOperations.title")}
          </h3>
          <div className="flex items-center justify-between">
            <p className="max-w-[390px] text-[16px] font-roboto text-wrap leading-[100%] text-blue-dark font-normal">
              {t("admin.security.sections.sensitiveOperations.description")}
            </p>
            <button
              ...
              className={`relative inline-flex h-6 min-w-11 items-center rounded-full transition-colors ${
                sensitiveOps ? "bg-[#EF4444]" : "bg-[#E5E7EB]"
              }`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
          ...
          <div className="text-[14px] leading-[400%] text-dark-gray">
            <span>
              {t("admin.security.sections.sensitiveOperations.preventionNote")}
            </span>
          </div>
        </section>
        ...
        <section className="border-t pt-6 px-8">
          <div className="flex items-center justify-between mb-[30px]">
            <h3 className="text-[20px] leading-[100%] font-bold text-oxford-blue ">
              {t("admin.security.sections.loginHistory.title")}
            </h3>
            <button className="px-3 py-2 rounded-md text-[16px] leading-4 font-bold w-[98px] h-9 border border-[#E5E7EB] flex gap-2 items-center justify-center">
              ...
              {t("admin.security.sections.loginHistory.filter")}
            </button>
          </div>
          ...
          <div className="flex gap-4 mb-4">
            <input
              placeholder={t(
                "admin.security.sections.loginHistory.filterByUser"
              )}
              className="border border-[#E5E7EB] px-3 py-2 rounded-md text-[14px] w-[344px] h-[50px]"
            />
            <input
              placeholder={t(
                "admin.security.sections.loginHistory.datePlaceholder"
              )}
              className="border border-[#E5E7EB] px-3 py-2 rounded-md text-[14px] w-[202px]"
            />
          </div>
          <LoginHistoryTable items={loginData} columns={loginColumns} />
        </section>

        {/* Save + Cancel */}
        <div className="flex justify-end gap-3 pt-6 px-8">
          <button className="border border-[#E5E7EB] text-[16px] leading-[24px] font-medium rounded-md h-9 w-[120px] text-blue-dark">
            {t("admin.security.actions.cancel")}
          </button>
          <button className=" bg-[#ED4122] rounded-md h-9 w-[158px] text-[16px] leading-[100%] font-medium text-white">
            {t("admin.security.actions.saveChanges")}
          </button>
        </div>
      </div>
    </div>
