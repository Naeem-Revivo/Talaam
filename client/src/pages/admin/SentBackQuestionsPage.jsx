...
const SentBackQuestionsPage = () => {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      // Fetch all questions to filter for:
      ...
      if (response.success && response.data) {
        ...
        const filteredQuestions = allQuestions.filter((q) => {
          ...
          const isPendingGatherer = q.status === 'pending_gatherer';
          
          // Check if created by admin and (flagged or rejected)
          const isCreatedByAdmin = q.createdBy?.adminRole === 'admin' || q.createdBy?.role === 'admin';
        ...
        const transformed = filteredQuestions.map((q) => {
        ...
        // Calculate summary from filtered questions
        const summaryData = {
          total: filteredQuestions.length,
          pending: filteredQuestions.filter(q => q.status === 'pending_gatherer' || q.status?.startsWith('pending_')).length,
          approved: filteredQuestions.filter(q => q.status === 'completed').length,
      ...
      <div className="mx-auto flex max-w-[1200px] flex-col gap-6">
        <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div className="space-y-2">
            <h1 className="font-archivo text-[36px] font-bold leading-[40px] text-oxford-blue">
              {t('admin.questionBank.sentBackQuestions.title')}
            </h1>
            <p className="font-roboto text-[18px] leading-[28px] text-[#6B7280]">
              {t('admin.questionBank.sentBackQuestions.subtitle')}
            </p>
          </div>
        </header>
        ...
        {loading ? (
          ...
          <QuestionBankTable
            ...
            onView={(question) => navigate(`/admin/question-bank/sent-back-questions/view?id=${question.id}`)}
          />
        )}
      </div>
    </div>
...
export default SentBackQuestionsPage;
